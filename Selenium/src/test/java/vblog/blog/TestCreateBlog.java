...
public class TestCreateBlog {
	...
	@Test
	public void create_blog_success() throws Exception {
		this.testContext.getWebDriverManager().getDriver()
				.get(FileReaderManager.getInstance().getConfigReader().getApplicatonUrl());
		Thread.sleep(1000);
		this.homePage.clickCreateBlog();
		Thread.sleep(1000);
		String title = "Title - " + LocalDateTime.now();
		String content = "Content - " + LocalDateTime.now();
